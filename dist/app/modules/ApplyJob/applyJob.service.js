...
exports.ApplyJobServices = void 0;
/* eslint-disable @typescript-eslint/no-explicit-any */
const mongoose_1 = __importDefault(require("mongoose"));
const job_model_1 = require("../Job/job.model");
const applyJob_model_1 = __importDefault(require("./applyJob.model"));
const createApplyJobIntoDB = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    console.log('payload :>> ', payload);
    const session = yield mongoose_1.default.startSession();
    try {
        session.startTransaction();
        // 🔹 Check Job exists
        const job = yield job_model_1.Job.findById(payload.jobInfo).session(session);
        if (!job) {
            throw new Error('Job not found');
        }
        // 🔹 Check already applied (IMPORTANT)
        const alreadyApplied = yield applyJob_model_1.default.findOne({
        ...
        // 🔹 Create application
        const application = yield applyJob_model_1.default.create([
            payload,
        ], { session });
        // 🔹 Increment applicant count
        yield job_model_1.Job.findByIdAndUpdate(payload.jobInfo, { $inc: { applicantCount: 1 } }, { session });
